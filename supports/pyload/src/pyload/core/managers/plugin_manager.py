    ...
    def parse_urls(self, urls):
        ...
        for url in urls:
            ...
            for name, value in chain(
                ...
                if not self.pyload.config.get_plugin(name, "enabled"):
                    continue

                if value["re"].match(url):
                    self.pyload.log.info(
                        "parse_urls: match succ:\n name:{}\n url:{}\npattern:{}\n".
                            format(name, url, value["re"]))
                    res.append((url, name))
                    last = (name, value)
                    found = True
                    break
