...
class ConfigParser:
    ...
    def add_plugin_config(self, name, config, outline=""):
        ...
        for item_name, item_value in config.items():
            if item_name in conf and item_value[0] == conf[item_name]["type"]:
                conf[item_name]["desc"] = item_value[1]
            else:
                conf[item_name] = {
                    "desc": item_value[1],
                    "type": item_value[0],
                    "value": self.cast(item_value[0], item_value[2]),
                }

        values = [x for x,k in config.items()] + ["desc", "outline"]
