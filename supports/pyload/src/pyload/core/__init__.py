...
from .. import PKGDIR, APPID, USERHOMEDIR
...
class Core:
    ...
    def _init_webserver(self):
        from ..webui.webserver_thread import WebServerThread
    ...
    def start(self):
        try:
            ...
            self.log.info(self._("User directory: {}\n").format(self.userdir))
            self.log.info(self._("Cache directory: {}\n").format(self.cachedir))

            storage_folder = self.config.get("general", "storage_folder")
            self.log.info(self._("Storage directory: {}\n".format(storage_folder)))

            avail_space = format.size(fs.free_space(storage_folder))
            self.log.info(self._("Storage free space: {}\n").format(avail_space))
