...
class HttpProxyMiddleware(object):
    ...
    def reset_proxyes(self):
        ...
        """
        log.info("reset proxyes to valid")
        for p in self.proxyes:
    ...
    def fetch_new_proxyes(self):
        ...
        从网上抓取新的代理添加到代理列表中
        """
        log.info("extending proxyes using fetch_free_proxyes.py")
        new_proxyes = []#fetch_free_proxyes.fetch_all(https=self.use_https)
        log.info("new proxyes: %s" % new_proxyes)
        self.last_fetch_proxy_time = datetime.now()
    ...
    def inc_proxy_index(self, current=-1):
        ...
        if self.len_valid_proxy() < self.extend_proxy_threshold: # 代理数量仍然不足, 抓取新的代理
            log.info("valid proxy < threshold: %d/%d" % (self.len_valid_proxy(), self.extend_proxy_threshold))
            self.fetch_new_proxyes()

        log.info("now using new proxy: %s" % self.proxyes[self.proxy_index]["proxy"])
        ...
        #    log.info("%d munites since last fetch" % self.fetch_proxy_interval)
        #    self.fetch_new_proxyes()
    ...
    def invalid_proxy(self, index):
        ...
        if index < self.fixed_proxy: # 可信代理永远不会设为invalid
            log.info("fixed proxy will not be invalid: %s" % self.proxyes[index])
            self.inc_proxy_index(index)
            return

        if self.proxyes[index]["valid"]:
            log.info("invalidate %s" % self.proxyes[index])
    ...
    def dump_valid_proxy(self):
        ...
        if self.dump_count_threshold <= 0:
            return
        log.info("dumping proxyes to file")
    ...
    def process_request(self, request, spider):
        ...
        if self.proxy_index > 0  and datetime.now() > (self.last_no_proxy_time + timedelta(minutes=self.recover_interval)):
            log.info("After %d minutes later, recover from using proxy" % self.recover_interval)
            self.last_no_proxy_time = datetime.now()
            self.proxy_index = 0
        ...
        if "change_proxy" in request.meta.keys() and request.meta["change_proxy"]:
            log.info("change proxy request get by spider: %s" % request)
    ...
    def process_response(self, request, response, spider):
        ...
        if "proxy" in request.meta.keys():
            log.debug("%s %s %s" % (request.meta["proxy"], response.status, request.url))
        else:
            log.debug("None %s %s" % (response.status, request.url))
        ...
        if response.status != 200 \
            ...
            log.info("response status[%d] not in spider.website_possible_httpstatus_list" % response.status)
            self.invalid_proxy(request.meta["proxy_index"])
    ...
    def process_exception(self, request, exception, spider):
        ...
        处理由于使用代理导致的连接异常
        """
        log.debug("%s exception: %s" % (self.proxyes[request.meta["proxy_index"]]["proxy"], exception))
